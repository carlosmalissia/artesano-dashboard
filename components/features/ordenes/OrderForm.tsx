  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <div className="mt-6">
          ...
          <div className="space-y-2">
            {form.watch('productos')?.map((item, index) => (
              <div key={index} className="flex justify-between bg-slate-800 p-3 rounded-md">
                <div>
        ...
        <Button type="submit" disabled={uploading || loading || isPagada}>
          {isPagada
            ? 'Factura Pagada'
            : modo === 'edit'
              ? uploading
